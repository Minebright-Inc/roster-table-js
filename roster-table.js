...
var timescale_changed = true;

var roster_rotation_type ="All";

var workgroup_mates = [];

var event_id_clicked = -1;
var user_id_clicked = -1;
var potential_events = [];
var potential_days_owed = 0;
var event_type_options = [];
var potential_cal_name = "";
var roster_status = "All";
var roster_country = "All";
var roster_city = "All";
var roster_department = "All";
var roster_workgroup = "All";
var roster_role = "All";
var roster_manager = "All";

var roster_type = "All";
var roster_user_type = "All";
var roster_company = "All";
var roster_active = "All";
var roster_users = [];
var roster_search = "";
/**
...
function postRosterChangeRequest(user_id, event_id, new_start, is_projected, from, to, comment, confirm_booking) {
...
function postNewEventRequest(user_id_clicked,
                            event_id_clicked,
                            start,
                            end,
                            event_type,
...
async function fetchAllCountries() {
  ...
  try {
    let data = await fetch("/api/countries", options);
...
async function fetchAllDepartments() {
  ...
  try {
    let data = await fetch("/api/departments", options);
    return await data.json();
  } catch (error) {
    console.log(error);
  }
...
async function fetchAllWorkgroups() {
  ...
  try {
    let data = await fetch("/api/workgroups", options);
    return await data.json();
  } catch (error) {
    console.log(error);
  }
...
async function fetchAllRoles() {
  ...
  try {
    let data = await fetch("/api/roles", options);
    return await data.json();
  } catch (error) {
    console.log(error);
  }
...
async function fetchUsersJSON(type, status, country, city ,manager=null, role=null, workgroup=null) {
  const params = {
...
function updateRoster(name, users, createFilter, createModal, is_potential_roster, is_review) {
  
  cleanBody(name);
  setLabels(name);
  createRosterBody(name, users, createFilter, createModal, is_potential_roster, is_review);
  cleanHead(name);
...
function changeToCurrentMonth(name, users, createFilter, createModal, is_potential_roster, is_review) {
  var today = new Date();
  current_month = today.getMonth();
  current_year = today.getFullYear();
  timescale_changed = true;
  updateRoster(name, users, createFilter, createModal, is_potential_roster, is_review);
}
 ...
 */
function changeToNextMonth(name, users, createFilter, createModal, is_potential_roster, is_review) {
  if (current_month == 11) {
    current_year++;
    current_month = 0;
  } else {
...
function changeToPrevMonth(name, users, createFilter, createModal, is_potential_roster, is_review) {
  if (current_month == 0) {
    current_year--;
    current_month = 11;
  } else {
 ...
 */
function daysInMonth(month, year, createFilter, createModal, is_potential_roster) {
...
function populateTableHeader(name) {
    ...
    var table_header = document.getElementById("table_header_" + name);
    ...
    table_header.style.cssText = "position: sticky; top: 0; background:white; z-index: 1;";

    $("<th style='width:100px ;'>Name</th>").appendTo(row);
    var days_owed_planned_header = document.createElement("th");
    days_owed_planned_header.innerHTML = "Planned ";
    days_owed_planned_header.setAttribute('style', 'writing-mode: vertical-lr; width: 65px; min-height: 60px;');
    ...
    var days_owed_actual_header = document.createElement("th");
    days_owed_actual_header.setAttribute('style', 'width: 65px; min-height: 60px;');
    days_owed_actual_header.innerHTML = "<p style='writing-mode: vertical-lr;'>Current</p>";
    days_owed_actual_header.setAttribute('class', 'has-tooltip-right');
    days_owed_actual_header.setAttribute('data-tooltip', 'The number of days of R&R owed to you from Company to this day. This only gets calculated up to today.');
    $(days_owed_actual_header).appendTo(row);

    for (var i = 1; i <= numDays; i++) {
        let day = new Date(current_year, current_month, i);
        
        let day_name = day.toLocaleString('default', { weekday: 'short' });
        var cell = $("<th style='padding:0; margin:0; min-width:35px; align-items: center;justify-content: center;' class = 'has-text-centered' >" +
                                                                                                         "<p>" + i + "</p>" +
                                                                                                         "<p>" + day_name + "</p>" +
                                                                                                         "</th>");
    ...
    if (
        ...
        $(row.cells[today.getDate() + 2]).attr("style", "padding:0; margin:0; min-width:35px; align-items: center;justify-content: center; border-right: 2px solid; border-left: 2px solid; border-top: 2px solid;");
    }
}
...
function deleteTableBody(name) {
  ...
  var nodes_length = table_body.childNodes.length;
  var nodes = table_body.childNodes;
...
function createInitialTable(name, height, width) {
  var main_div = document.getElementById(name);
  ...
  table.className = "table is-narrow has-sticky-header is-bordered is-striped is-hoverable is-fullwidth";
...
function creatDropDownSelectField(name, field, options, values) {
  ...
  var field_body_div = document.createElement('div');
  ...
  for (var i = 0; i < options.length; i++) {
    ...
    if (field == 'User') {
      ...
      option.innerHTML = toTitleCase(values[i].first_name + " " + values[i].last_name);
    } else {
...
function registerSelectChange(name, users, createFilter, createModal, is_potential_roster, is_review) {
  // get all select class elements
  var select_fields = document.getElementsByClassName("select_field");
...
if (false){
    var select_user = creatDropDownSelectField(name, 
                                              "User",
                                              users,
                                              users);
  ...
  var select_active_roster = creatDropDownSelectField(name,
                                                      "Active Roster",
                                                      ["Yes", "No"],
                                                      ["Yes", "No"]);
  var column = document.createElement('div');
  ...
  var search_field_control = document.createElement('p');
  search_field_control.classList.add('control', 'has-icons-left');

  var search_icon = document.createElement('span');
  search_icon.classList.add('icon', 'is-small', 'is-left');
  var search_icon_img = document.createElement('i');
 ...
 * @param users - an array of user objects
 */
function createButtons(name, users, createFilter, createModal, is_potential_roster, is_review) {
  var main_container = document.getElementById(name);
  var container = document.createElement("div");
  $(container).addClass("field has-addons");

  var control_prev = document.createElement("p");
  ...
  var icon_prev = document.createElement("span");
  ...
  var control_cur = document.createElement("p");
  control_cur.className = "control";
  var button_cur = document.createElement("button");
  ...
  var icon_next = document.createElement("span");
  ...
  button_prev.addEventListener("click", function () {
    deleteNotification();
    changeToPrevMonth(name, users, createFilter, createModal, is_potential_roster, is_review);
  });
  button_next.addEventListener("click", function () {
    deleteNotification();
    changeToNextMonth(name, users, createFilter, createModal, is_potential_roster, is_review);
  });
  button_cur.addEventListener("click", function () {
    deleteNotification();
    changeToCurrentMonth(name, users, createFilter, createModal, is_potential_roster, is_review);
  });
...
function setLabels(name) {
  var label_range = document.getElementById("roster_date_label_" + name);
  var numDays = daysInMonth(current_month, current_year);
  var month = current_month;
  var monthNames = ["January", "February", "March", "April", "May", "June",
  "July", "August", "September", "October", "November", "December"
];
  label_range.innerHTML =
    monthNames[month] + ' '+
    current_year;
}
...
function populateRosterRow(row, user, type, result, days_owed, days_owed_actual, month, year, createModal, name, offset, last_roster)
{
    var numDays = daysInMonth(current_month, current_year);
    ...
    for (var d = 1; d <= numDays; d++) {
        ...
        if (today.getFullYear() == year && today.getMonth() == month && today.getDate() == d) {
            ...
            if (last_roster)
                x.setAttribute("style", "padding: 0; margin:0; min-height: 50px; width:25px; border-right: 2px solid; border-left: 2px solid; border-bottom: 2px solid;");
            else
                x.setAttribute("style", " padding: 0; margin:0; min-height: 50px; width:25px; border-right: 2px solid; border-left: 2px solid;");
        } else {
            x.setAttribute("style", "padding: 0; margin:0; min-height: 50px; width:25px;");
        }
        if (createModal == true && type == 'planned') {
    ...
    var num_items = result?.length || 0;
    for (var i = 0; i < num_items; i++) {
        var event = result[i];
        var event_type = event.type;
        let parsed = event.start.split("-");
        var start = new Date(parsed[2], parsed[1] - 1, parsed[0]);
        parsed = event.end.split("-");
        var end = new Date(parsed[2], parsed[1] - 1, parsed[0]);
        var index = 1;
        var color="";

        if (type == "potential") {
            if ((start < new Date(year, month, 1) && end < new Date(year, month, 1)) ||
                (start > new Date(year, month, numDays) && end > new Date(year, month, numDays))) {
                continue;
            }
        }
        ...
        if (start.getFullYear() < year) {
            ...
            index = Math.round((new_date.getTime() - start.getTime()) / (3600 * 1000 * 24))   + 1;
        ...
        for (var d = start.getDate(); d <= end.getDate(); d++) {
            d_in = d + 2 - offset;
            var num_events = parseInt(row.cells[d_in].getAttribute('data-num-events'));
            row.cells[d_in].setAttribute('data-num-events', num_events + 1);

            var text_color = pickTextColorBasedOnBgColorSimple(event_type.color, '#FFFFFF', '#000000');


            var tag =$("<span class='tag my-1 mx-0' style=' width:90%; background-color: " + event_type.color + "; color:" + text_color + ";'></span>");
            tag.attr('event_id', event.id);
            
            tag.addClass('has-tooltip-arrow has-tooltipl-multiline');
            tag.attr('data-tooltip', "Type: " + event_type.name + 
                                     "\nStart: " + event.start + 
                                     "\nEnd: " + event.end +
            ...
            if (event_type.show_icon == true && event_type.icon != '')
                tag.append("<span class='icon'><i class='" + event_type.icon + "'></i></span>");
            else
                tag.text(index);
            

            if (event.is_flight){
                tag.attr('style', tag.attr('style').replace('background-color: ' + event_type.color, 'background-color: '+ event.color));
                tag.attr('style', tag.attr('style').replace('color: ' + text_color, 'color: '+ pickTextColorBasedOnBgColorSimple(event.color, '#FFFFFF', '#000000')));
                tag.attr('data-tooltip', tag.attr('data-tooltip') + 
                                        "\nOrigin: " + event.origin +
                                        "\nDestination: " + event.destination +
                                        "\nIs Inbound: " + event.is_inbound +
                                        "\nIs Ad Hoc: " + event.is_adhoc +
                                        "\nConfirmed: " + event.confirmed +
                                        "\nStatus: " + event.status.status);
                tag.empty();
                if (event.is_inbound)
                    tag.append("<span class='icon'><i class='fa-solid fa-plane-arrival'></i></span>");
                else
                    tag.append("<span class='icon'><i class='fa-solid fa-plane-departure'></i></span>");
...
function createUserRoster(
  ...
  var days_owed_planned = Math.round(user.days_owed_planned * 100) / 100;
  var days_owed_actual = Math.round(user.days_owed_actual * 100) / 100;

  var name_cell = document.createElement("th");
    name_cell.setAttribute("style", "width: 180px; overflow: hidden; text-overflow: ellipsis;white-space: nowrap; ");
  var link_to_user = document.createElement("a");
  link_to_user.setAttribute('style', 'width:180px%; overflow: hidden;text-overflow: ellipsis; white-space: nowrap;');
  ...
  if (potential_roster != null) {
    ...
    populateRosterRow(row_potential, user, "potential", potential_roster, potential_days_owed, days_owed_actual, month, year, createModal, name, offset, false);
    offset = 1;
  }
  ...
  if (roster_type == "All" || roster_type == "planned") {
    ...
    var row_planned = document.createElement("tr");
...
function filterUsers(users){
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    if(add == true && roster_search != ""){
      if (user.full_name.toLowerCase().indexOf(roster_search.toLowerCase()) > -1) {
        add = true;
      } else {
        add = false;
      }
...
async function getUsersEvents(users) {
  ...
  for (var i = 0; i < users.length; i++) {
    var user = users[i];
    var user_events = await getUserEvents(user.id);
 ...
 */
function createRosterBody(name, users, createFilter, createModal, is_potential_roster, is_review) {

  var table_body = document.getElementById("table_body_" + name);
  var filtered = users;
  var events =  [];
  var potential_roster = null;
  ...
  for (var i = 0; i < filtered.length; i++) {
    var result = matchEventToUser(filtered[i], current_events);
...
function getAllEventsFromCell(cell){
  var event_nodes  = cell.childNodes;
  var events = [];
...
function resetDeleteSectionDropdown(name){
  var select = document.getElementById("delete_select_" + name);
...
function populateDropdownDeleteDiv(events_in_cell, all_events, name){
  ...
  for (var i = 0; i < events_in_cell.length; i++){
          var option = document.createElement("option");
          var event = getEventByIdFromList(events_in_cell[i], all_events);
...
function populateDeleteEventDiv(cell, all_events, name){
  ...
  var div = document.getElementById('delete_event_div');

  var events = getAllEventsFromCell(cell);
...
function populateNewEventDiv(user_id, year, month, date, name){
    var start_select = document.getElementById("new_event_modal_start_date_" + name);  
    var end_select = document.getElementById("new_event_modal_end_date_" + name);
    var departure_select = document.getElementById("new_event_modal_flight_departure_date_" + name);
    var return_select = document.getElementById("new_event_modal_flight_return_date_" + name);
    let parsed_date = new Date(year, month, date);
...
function resetAdhocWarning(){
    var warning = document.getElementById("request_roster_adhoc_warning");
    warning.innerText = "";
...
function createModalDeleteSection(div_delete, name)
{
  ...
  var dropdown_field = document.createElement("div");
  dropdown_field.classList.add("field");
  var dropdown_control = document.createElement("div");
  ...
  var event_to_delete_name_field = document.createElement("div");
  event_to_delete_name_field.classList.add("field");
  var event_to_delete_name_label = document.createElement("label");
  ...
  event_to_delete_name_paragraph.classList.add('title', 'is-3', 'has-text-link');
  ...
  var event_to_delete_type_field = document.createElement("div");
  event_to_delete_type_field.classList.add("field");
  var event_to_delete_type_label = document.createElement("label");
  ...
  event_to_delete_type_paragraph.classList.add('title', 'is-3', 'has-text-link');
  ...
  var event_to_delete_start_date_field = document.createElement("div");
  event_to_delete_start_date_field.classList.add("field");
  var event_to_delete_start_date_label = document.createElement("label");
  event_to_delete_start_date_label.classList.add("label");
  event_to_delete_start_date_label.innerHTML = "Event's start date:";
  event_to_delete_start_date_field.appendChild(event_to_delete_start_date_label);
  event_to_delete_start_date_paragraph = document.createElement("p");
  event_to_delete_start_date_paragraph.classList.add('title', 'is-3', 'has-text-link');
  event_to_delete_start_date_paragraph.setAttribute("id", "event_to_delete_start");
  event_to_delete_start_date_field.appendChild(event_to_delete_start_date_paragraph);
  div_delete.appendChild(event_to_delete_start_date_field);

  // Event to delete end date
  var event_to_delete_end_date_field = document.createElement("div");
  event_to_delete_end_date_field.classList.add("field");
  var event_to_delete_end_date_label = document.createElement("label");
  ...
  event_to_delete_end_date_paragraph.classList.add('title', 'is-3', 'has-text-link');
  event_to_delete_end_date_paragraph.setAttribute("id", "event_to_delete_end");
  event_to_delete_end_date_field.appendChild(event_to_delete_end_date_paragraph);
  div_delete.appendChild(event_to_delete_end_date_field);

  // Confirm Delete
  var confirm_delete_div = document.createElement("div");
  ...
  var confirm_delete_input = document.createElement("input");
  confirm_delete_input.setAttribute("type", "checkbox");
  confirm_delete_input.setAttribute("name", "confirm_delete");
  confirm_delete_input.setAttribute("id", "new_event_modal_confirm_delete_" + name);
  ...
  delete_btn.addEventListener("click", function() {
        deleteNotification();
          if (confirm_delete_input.checked){
            var event_id = dropdown_select.value;
  ...
  dropdown_control.addEventListener("change", function() {
          ...
          var event_id = document.getElementById('delete_select_' + name).value;
          if (event_id != "-1" ){
            $.ajax({
...
function createModalNewEventSection(div_new, name){
    ...
    //Event Type
    var ev_type_container = document.createElement("div");
    var event_type_div = document.createElement("div");
    event_type_div.className = "select is-medium is-fullwidth";
    var event_type_label = document.createElement("label");
    event_type_label.className = "label";
    event_type_label.innerHTML = "Event Type";

    var event_type_select = document.createElement("select");
    event_type_select.setAttribute("id", "event_type_select_" + name);
    event_type_select.setAttribute("name", "event_type_select_" + name);
    for (var i = 0; i < event_type_options.length; i++) {
            var event_type_option = document.createElement("option");
            event_type_option.setAttribute("value", event_type_options[i].name);
            event_type_option.innerHTML = event_type_options[i].name;
            event_type_option.setAttribute("id", "event_type_option_" + name + "_" + event_type_options[i].id);
    ...
    var start_label = document.createElement("element");
    ...
    var end_label = document.createElement("element");
    ...
    //Flight switch
    var flight_switch_div = $('<div>').addClass('field');
    var flight_switch_input = $('<input>').addClass('switch is-rounded is-outlined is-link').attr('type', 'checkbox').attr('id', 'new_event_modal_flight_switch_' + name)
    var flight_switch_label = $('<label>').attr('for', 'new_event_modal_flight_switch_' + name);
    ...
    //Flight switch
    var flight_round_trip_switch_div = $('<div>').addClass('field is-hidden');
    var flight_round_trip_switch_input = $('<input>').addClass('switch is-rounded is-outlined is-link').attr('type', 'checkbox').attr('id', 'new_event_modal_flight_round_trip_switch_' + name).attr('checked', 'checked');
    var flight_round_trip_switch_label = $('<label>').attr('for', 'new_event_modal_flight_round_trip_switch_' + name);
    ...
    var flights_div = $('<div>').addClass('columns is-hidden');
    var flight_departure_div = $('<div>').addClass('column is-half');
    var flight_return_div = $('<div>').addClass('column is-half');

    //Flight Departure box
    var flight_departure_box = $('<div>').addClass('box');
    var flight_departure_box_label = $('<label>').addClass('label').text('Departure Flight');
    flight_departure_box.append(flight_departure_box_label);

    var flight_departure_from = $('<div>').addClass('field');
    var flight_departure_from_label = $('<label>').addClass('label').text('From');
    var flight_departure_from_input = $('<input>').addClass('input is-medium').attr('type', 'text').attr('name', 'flight_departure_from').attr('id', 'new_event_modal_flight_departure_from_' + name);
    ...
    var flight_departure_to = $('<div>').addClass('field');
    var flight_departure_to_label = $('<label>').addClass('label').text('To');
    var flight_departure_to_input = $('<input>').addClass('input is-medium').attr('type', 'text').attr('name', 'flight_departure_to').attr('id', 'new_event_modal_flight_departure_to_' + name);
    ...
    var flight_departure_date = $('<div>').addClass('field');
    var flight_departure_date_label = $('<label>').addClass('label').text('Arrival Date');
    var flight_departure_date_input = $('<input>').addClass('input is-medium').attr('type', 'date').attr('name', 'flight_departure_date').attr('id', 'new_event_modal_flight_departure_date_' + name);
    ...
    //Flight Return box
    var flight_return_box = $('<div>').addClass('box');
    var flight_return_box_label = $('<label>').addClass('label').text('Return Flight');
    flight_return_box.append(flight_return_box_label);

    var flight_return_from = $('<div>').addClass('field');
    var flight_return_from_label = $('<label>').addClass('label').text('From');
    var flight_return_from_input = $('<input>').addClass('input is-medium').attr('type', 'text').attr('name', 'flight_return_from').attr('id', 'new_event_modal_flight_return_from_' + name);
    ...
    var flight_return_to = $('<div>').addClass('field');
    var flight_return_to_label = $('<label>').addClass('label').text('To');
    var flight_return_to_input = $('<input>').addClass('input is-medium').attr('type', 'text').attr('name', 'flight_return_to').attr('id', 'new_event_modal_flight_return_to_' + name);
    ...
    var flight_return_date = $('<div>').addClass('field');
    var flight_return_date_label = $('<label>').addClass('label').text('Return Date');
    var flight_return_date_input = $('<input>').addClass('input is-medium').attr('type', 'date').attr('name', 'flight_return_date').attr('id', 'new_event_modal_flight_return_date_' + name);
    ...
    submit_btn.addEventListener("click", function() {
        var start_date_array = input_start_date.value.split("-");
        var start_date = new Date(start_date_array[0], start_date_array[1] - 1, start_date_array[2]);

        var end_date_array = input_end_date.value.split("-");
        var end_date = new Date(end_date_array[0], end_date_array[1] - 1, end_date_array[2]);

        var dep_date_array = flight_departure_date_input.val().split("-");
        var dep_date = new Date(dep_date_array[0], dep_date_array[1] - 1, dep_date_array[2]);

        var ret_date_array = flight_return_date_input.val().split("-");
        var ret_date = new Date(ret_date_array[0], ret_date_array[1] - 1, ret_date_array[2]);
        ...
        if (flight_switch_input.is(':checked')) {
            if (flight_departure_from_input.val() == "" || flight_departure_to_input.val() == "" || flight_departure_date_input.val() == "") {
                createNotification("Please fill in all the fields for the flight", 'error');
                return;
            }
            if (flight_round_trip_switch_input.is(':checked')) {
                if (flight_return_from_input.val() == "" || flight_return_to_input.val() == "" || flight_return_date_input.val() == "") {
                    createNotification("Please fill in all the return fields for the flight", 'error');
                    return;
                }
            }
        }
        ...
        deleteNotification();
        postNewEventRequest(user_id_clicked= user_id_clicked, 
...
function createFlightRequestBox(box_name, label_name){
  ...
  var label_cols = document.createElement('div');
  label_cols.classList.add('columns');
  var label_col = document.createElement('div');
  ...
  // Origin Column
  var origin_cols = document.createElement('div');
  origin_cols.classList.add('columns');
  var origin_column = document.createElement('div');
  origin_column.classList.add('column');
  origin_cols.appendChild(origin_column);
  var origin_label = document.createElement('label');
  ...
  var destination_cols = document.createElement('div');
  destination_cols.classList.add('columns');
  var destination_column  = document.createElement('div');
  destination_column.classList.add('column');
  destination_cols.appendChild(destination_column);
  var destination_label = document.createElement('label');
  ...
  var departure_date_cols = document.createElement('div');
  departure_date_cols.classList.add('columns');
  var dep_date_col = document.createElement('div');
  dep_date_col.classList.add('column');
  var dep_date_label = document.createElement('label');
  dep_date_label.classList.add('label');
  dep_date_label.innerText = 'Departure Date';
  var dep_date_input = document.createElement('input');
...
function createModalAdhocSection(div_adhoc, name){
    ...
    var cols_header = document.createElement('div');
    cols_header.classList.add('columns');
    var col_header = document.createElement('div');
    ...
    // Warning box
    var cols_0 = document.createElement('div');
    ...
    warning_box.classList.add('notification', 'is-warning', 'is-hidden', 'has-text-centered');
    ...
    var cols_1 = document.createElement('div');
    ...
    var cols_3 = document.createElement('div');
    ...
    submit_btn.addEventListener('click', function(){
        ...
        if (from_field_in.value == "" || to_field_in.value == "" || from_date_in == ""){
            resetAdhocWarning();
            populateAdhocWarning('warning', 'Please fill in desired flight details');
        return;
        }
        if (from_field_out.value == "" || to_field_out.value == "" || from_date_out == ""){
            if (confirm('You have not filled in the return flight details. Are you sure you want to submit?') == false){
                return;
            }
        }
        $.ajax({
...
function createTravelChangeRequestSection(div_travel, users, name, potential_roster_name){
  ...
  var label_travel_header_div = document.createElement("div");
  label_travel_header_div.classList.add("field", "has-text-centered");
  var label_travel_header = document.createElement("label");
  ...
  group_field.classList.add('field', 'is-grouped', 'is-horizontal', 'is-grouped-centered');
  ...
  input_start_date.addEventListener("change", function() {
          var start_date = input_start_date.value;
          var start_date_array = start_date.split("-");
          var is_projected = input_is_projected.checked;

          var date = new Date(start_date_array[0], start_date_array[1] - 1, start_date_array[2]);
          getPotentialEventsJSON(user_id_clicked, event_id_clicked, date.toLocaleDateString('en-GB'), is_projected);
          timescale_changed = true;
          updateRoster(potential_cal_name, users, false, false, true, false);
  });

  // Select is_projected Field
  var is_projected_div = document.createElement("div");
  is_projected_div.classList.add("field", 'is-fullwidth');
  is_projected_div.setAttribute('style', 'width: 100%');
  var is_projected_label = document.createElement("element");
  ...
  var group_from_to_field = document.createElement('div');
  group_from_to_field.classList.add('field', 'is-grouped', 'is-horizontal', 'is-grouped-centered', 'field-body');
  ...
  var from_label = document.createElement("element");
  ...
  var to_label = document.createElement("element");
  ...
  var comment_label = document.createElement("element");
  comment_label.classList.add("label");
  comment_label.innerHTML = "Reason for change: ";
  var input_comment = document.createElement("textarea");
  ...
  submit_btn.addEventListener("click", function() {
          ...
          var confirm_booking = confirm('Do you want to send this request for booking with the Travel Team?')
          var start_date = input_start_date.value;
          var start_date_array = start_date.split("-");
          var date = new Date(start_date_array[0], start_date_array[1] - 1, start_date_array[2]);
          var from = input_from.value;
          var to = input_to.value;
          var comment = input_comment.value;
          deleteNotification();
          postRosterChangeRequest(user_id_clicked, event_id_clicked, date.toLocaleDateString('en-GB'), is_projected, from, to, comment, confirm_booking);
  });
...
{
  var height = users.length * 300;
  createCalendar(potential_roster_name, users, 1600, height, false, false, true, false);
  
}


function createNewEventModal(name, users, createFilter, createModal, is_potential_roster, is_review, potential_roster_name) {
  var container = document.getElementById(name);
  ...
  div_delete.classList.add("is-hidden", 'box', 'has-background-link-light', 'has-text-centered');
...
//   div_adhoc.classList.add("is-hidden", 'box', 'has-background-link-light', 'has-text-centered');
  ...
  var div_travel = document.createElement("div");
  div_travel.setAttribute("id", "travel_request_div");
  div_travel.classList.add("is-hidden", 'box', 'has-background-link-light', 'has-text-centered');
  ...
  var modal_card_header = document.createElement("header");
  modal_card_header.className = "modal-card-head";
  var modal_card_header_title = document.createElement("p");
  ...
  var modal_card_body = document.createElement("section");
  modal_card_body.className = "modal-card-body has-text-black";

  var modal_card_footer = document.createElement("footer");
 ...
 // Tabs section
  ...
  var sections = ["Request travel change", 'Request a non-roster event/travel', 'Request event deletion'];
  var sec_ids = ['travel_request_section', 'new_event_modal_create_section', 'new_event_modal_delete_section'];
  ...
  for (var i = 0; i < sections.length; i++) {
    ...
    tab_a.addEventListener('click', function(e) {
      var tab_li = e.target.parentNode;
      var tabs = document.getElementsByClassName("tab_li");
      deleteNotification();
      ...
      tab_li.classList.add("is-active");
      var tab_id = tab_li.getAttribute("id");
...
//   modal_card_body.appendChild(div_adhoc);
  div_travel = createTravelChangeRequestSection(div_travel, users, name, potential_roster_name);
...
function createCalendar(name,
  ...
  timescale_changed = true;
  if (createFilter == true) {
    createFilters(name, users, createFilter, createModal, is_potential_roster, is_review, countries, workgroups, departments, roles, roster_types, user_types, companies);
  }

  createButtons(name, users, createFilter, createModal, is_potential_roster, is_review);
  ...
  if (createModal == true) {
    ...
    getEventTypes();
    createNewEventModal(name, users, createFilter, createModal, is_potential_roster, is_review, potential_roster_name);
  }
  populateTableHeader(name);
